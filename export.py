...
def success_message(message):
    msg = {"SUCCESS": {"time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                   "message": message}}
...
def error_message(message):
    msg = {"WARNING": {"time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                 "message": message}}
...
def load_to_sql(dataframe, engine_function, database_url):
    ...
    print("Exporting dataframe to SQL...")
    ...
    try:
        ...
        if exists and sql_row_num.iloc[0, 0] == len(dataframe) and sql_col_num.iloc[0, 0] == len(dataframe.columns) and not skip_load:
            ...
            error_message("The Dataframe already exists in SQL, export stopped")
            skip_load = True
            ...
            success_message("Dataframe successfully exported to SQL")
...
def create_summaries(engine, db_url):
    ...
    if not exists1 and not exists2 and not exists3:
        with engine(db_url).begin() as conn:
            ...
            success_message("Summary tables created in Postgres")


    if exists1 and exists2 and exists3:
        error_message("Summary tables already exist, skipping load")
        print("Summary tables already exist, skipping load")
